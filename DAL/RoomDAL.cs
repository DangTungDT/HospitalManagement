...
namespace DAL
{
    public class RoomDAL
    {
        ...
        public bool CapnhatRoom(RoomDTO dtoroom)
        {
            try
            {
                if (db.Rooms.Where(x => x.id == dtoroom.ID1).FirstOrDefault() != null)
                {
                    var update = db.Rooms.Single(sp => sp.id == dtoroom.ID1);
                    update.roomName = dtoroom.RoomName;
                    update.bedCount = dtoroom.BedCount;
                    update.departmentID = dtoroom.DepartmentID;
                    db.SubmitChanges();
                    return true;
                }
                return false;
