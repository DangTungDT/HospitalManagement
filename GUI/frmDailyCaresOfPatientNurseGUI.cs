...
namespace GUI
{
    public partial class frmDailyCaresOfPatientNurseGUI : Form
    {
        public frmDailyCaresOfPatientNurseGUI(string patientId)
        {
            InitializeComponent();
        ...
        private void frmDailyCaresOfPatientNurseGUI_Load(object sender, EventArgs e)
        {
            dgvDailyCare.DataSource = bll.GetDailyCaresByPatient(patientId);
            dgvDailyCare.Columns["DateCare"].HeaderText = "Ngày chăm sóc";
            dgvDailyCare.Columns["Shift"].HeaderText = "Ca";
            dgvDailyCare.Columns["BloodPressure"].HeaderText = "Huyết áp";
            dgvDailyCare.Columns["BodyTempearature"].HeaderText = "Nhiệt độ (°C)";
            dgvDailyCare.Columns["PulseRate"].HeaderText = "Mạch (lần/phút)";
            dgvDailyCare.Columns["Note"].HeaderText = "Ghi chú";
            dgvDailyCare.Columns["NurseName"].HeaderText = "Người chăm sóc";
            dgvDailyCare.Columns["RoomName"].HeaderText = "Phòng";

            // Ẩn các cột không cần thiết
            dgvDailyCare.Columns["Id"].Visible = false;
            dgvDailyCare.Columns["PatientID"].Visible = false;
            dgvDailyCare.Columns["RoomID"].Visible = false;
            dgvDailyCare.Columns["NurseID"].Visible = false;
            dgvDailyCare.Columns["PatientName"].Visible = false;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Không tìm thấy bệnh nhân của bác sĩ!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            FormDailyCaresByPatientReporstNurseGUI frmReport = new FormDailyCaresByPatientReporstNurseGUI(patientId);
